...
import { REGISTRY_ADDRESS } from "@/lib/contracts/registry";
      ...
      <div>
        <div className="flex items-center justify-between px-4 py-6 sm:px-6 md:space-x-10">
          ...
          <div>
            ...
            <NavLink path="https://github.com/PaulRBerg/prb-proxy" className="text-hover" label="Documentation" />
          </div>
        </div>
      </div>
        ...
        <div className="mx-auto h-auto w-auto rounded-full text-center">
          ...
          <div className="flex items-center">
            <pre className="text-sm">
              <code className="font-semibold">{REGISTRY_ADDRESS}</code>
            </pre>
            <ClipboardDocumentIcon className="ml-3 h-4 w-4 cursor-pointer" onClick={() => onCopy(REGISTRY_ADDRESS)} />
          </div>
        </div>
      </div>
    </header>
